...
export const Event: React.FC = () => {
    ...
    useEffect(() => {
        const fetchTrailPoints = async () => {
            const response = await fetch('https://orkad-xyaaa-aaaal-ai7ta-cai.icp0.io/trails/Williams%20Peak%20via%20Williams%20Ridge%20Trail.gpx');
            const gpxData = await response.text();
            const points = parseGPX(gpxData);

            setTrailPoints(points.map(p => [p.latitude, p.longitude]));
        };
    ...
    return (
        <div className="event-page">
            <h1>Event: {eventDetails?.trailName}</h1>
